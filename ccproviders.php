<?php
/**
 * Custom query for looping through all available posts
 * of the custom post type 'Coin Providers' (ccproviders)
 */
...
// Use $paged as a value for the 'paged' parameter
// to display posts from the current page
// and set the 'paged' parameter to 1 when the query variable is not set (first page).
$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;

$args = array(
	'post_type'         => 'ccproviders',
	'post_status'       => 'publish',
	'posts_per_page'    => 10,
	// use 'provider_num' as the 'meta_key' value
	// that's the 'Provider Number' custom field value
	// that will also be displayed in a div with the `.ccp-num-circle` class next to provider title
	'meta_key'          => 'provider_num',
	// cast the 'meta_key' value as 'NUMERIC'
	// because the output of 'provider_num' will be a string
	// and that would order the posts as 1, 11, 2, 22 etc.
	'meta_type'         => 'NUMERIC',
...
// The Query
$ccproviders_query = new WP_Query( $args );
// `$paged` is the current page number
// `$ccproviders_query->max_num_pages` is the total number of pages
?>

<?php
if( $ccproviders_query->have_posts() ) :
?>
	...
	<!-- WP posts while loop END -->
        ...
        <!-- Load More button -->
        <?php
        // Show 'load more' button if there's more than one page of providers
        if ( $ccproviders_query->max_num_pages > 1 ) : ?>

        <div class="row loadmore">
            <div class="col-12 text-center mb-5 px-0">
                <button id="loadmore" class="btn btn-outline-info btn-block py-4" data-toggle="modal" data-target="#loadmoreModal">
                    SHOW MORE PROVIDERS
                </button>
                <div id="ajax-no-posts">
                    No more coin providers available. Check back later!
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="loadmoreModal" tabindex="-1" role="dialog" aria-labelledby="loadmoreModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-body">
                        If I had more time, I would have turned this into a nice 'load more' AJAX button.
                        But hopefully, what I've done here already demonstrates my skill set pretty well.
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    </div>
...
<?php
	...
	echo '<h2>There are no coin providers yet!</h2>';
endif;
?>
